...
pub async fn create_postgres_repository(
    ...
    let repository =
        create_postgres_oauth_client_repository(&database, runtime)
            .await
            .map_err(|error| match error {
                localid_oauth_client_repository_postgres::PostgresOAuthClientRepositoryError::Database(error) => error,

                _ => sqlx::Error::Protocol(
                    "invalid postgres repository data".into(),
                ),
            })?;
